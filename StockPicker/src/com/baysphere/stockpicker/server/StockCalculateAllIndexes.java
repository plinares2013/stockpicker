...
//	int rangeIndex = 3;
	...
	public void doGet (HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (allIndexesUpdated == false) {
			...
			//handle = defaultQueue.add(TaskOptions.Builder.withUrl("/stockpicker/stockCalculateAllIndexes").param("cursor", cursorString).taskName(taskName).countdownMillis(300000));		
			handle = defaultQueue.add(TaskOptions.Builder.withUrl("/stockpicker/stockCalculateAllIndexes").param("cursor", cursorString).countdownMillis(300000));
		}
		...
		if (allIndexesUpdated == true) {
		Queue defaultQueue = QueueFactory.getDefaultQueue();
				TaskHandle handle = defaultQueue.add(TaskOptions.Builder.withUrl("/stockpicker/getTopNIndexStocks").countdownMillis(600000)
