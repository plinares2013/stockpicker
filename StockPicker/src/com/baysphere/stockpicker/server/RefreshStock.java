	...
	public void doGet (HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		//}
			...
			if (SystemProperty.environment.value() == SystemProperty.Environment.Value.Development) {
				logger.log(Level.INFO,"Development mode: Queue name is " + queue);
			}
	    ...
	    //handle = defaultQueue.add(TaskOptions.Builder.withUrl("/stockpicker/backends/refreshStock2"));
		...
		if (allStocksUpdated == false) {
			...
			//handle = defaultQueue.add(TaskOptions.Builder.withUrl("/stockpicker/cron/refreshStock").param("cursor", cursorString).taskName(taskName).countdownMillis(delayRefreshStock));
			//handle = defaultQueue.add(TaskOptions.Builder.withUrl("/stockpicker/cron/refreshStock").param("cursor", cursorString).countdownMillis(delayRefreshStock));
			handle = defaultQueue.add(TaskOptions.Builder.withUrl("/stockpicker/backends/refreshStock").param("cursor", cursorString).countdownMillis(delayRefreshStock)
		...
		*/
		...
		//After all stocks have been updated launch the process of calculating the index for all stocks after a 20 minute delay
		 ...
		 handle = defaultQueue.add(TaskOptions.Builder.withUrl("/stockpicker/stockCalculateAllIndexes").countdownMillis(600000).method(TaskOptions.Method.valueOf("GET")));
		 }
	}
